// Derived from emilwidlund/ASCII (MIT, per upstream package.json).
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  // Use character luminance as alpha so the background stays transparent.
  asciiCharacter.a = asciiCharacter.r * pixelized.a;
...
export class ASCIIEffect extends Effect {
  ...
  private createCharactersTexture(characters: string, fontSize: number): Texture {
    ...
    if (!context) {
      throw new Error('ASCIIEffect: 2D context not available')
    }
  ...
  setCellSize(cellSize: number) {
    ...
    if (uniform) uniform.value = cellSize
  }

  setInvert(invert: boolean) {
    const uniform = this.uniforms.get('uInvert')
    if (uniform) uniform.value = invert
  }

  setColor(color: string) {
    const uniform = this.uniforms.get('uColor')
    if (uniform && uniform.value && typeof (uniform.value as Color).set === 'function') {
      ;(uniform.value as Color).set(color)
    }
  }
}
